...
export default function EditPartModal({ part, onClose, onUpdate, open = false }) {
  ...
  useEffect(() => {
    if (part && open) {
      const partUnitPrice = part.unitPrice || 0;
      const partMarkup = part.markupPercentage || 20;
      const partSalesPrice = part.price || 0;
      ...
      form.setFieldsValue({
        ...
        markupPercentage: partMarkup,
        price: partSalesPrice,
        minStock: part.minStock,
        status: part.status,
        supplier: part.supplier || '',
        description: part.description || ''
      });
      setUnitPrice(partUnitPrice);
      setMarkupPercentage(partMarkup);
      setSalesPrice(partSalesPrice);
                ...
                validator: async (_, value) => {
                  ...
                  try {
                    ...
                    const duplicate = snapshot.docs.some(doc => {
                      const data = doc.data();
                      // Don't check against the current part being edited
                      if (doc.id === part?.id) return false;
