...
export default function EditPartsRequestModal({ car, customer, onSubmit, onClose, open }) {
  const { user } = useAuth();
  ...
  return (
    <Modal
      title="Request Parts for Edit Report"
      open={isOpen}
    ...
    >
      ...
      <div style={{ marginBottom: 16, padding: 12, background: '#f7fafc', borderRadius: 8 }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, fontSize: 13 }}>
          ...
          <Text><b>Mobile:</b> {customer?.phoneNumber || 'N/A'}</Text>
          <Text><b>Vehicle:</b> {car ? `${car.year} ${car.make} ${car.model}` : 'N/A'}</Text>
          <Text><b>Plate:</b> {car?.plateNumber || 'N/A'}</Text>
          <Text><b>Mechanic Head:</b> {user?.displayName || 'Unknown'}</Text>
        </div>
      </div>
      ...
      >
        <Form.Item 
          label="Mechanic Name" 
          name="mechanicName"
          rules={[{ required: true, message: 'Please enter the mechanic name' }]}
        >
